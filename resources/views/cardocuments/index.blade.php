                    ...
                    <tbody>
                        @foreach($cardocuments as $item)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ url('/cardocuments/' . $item->id) }}" title="View document">
                                        <button class="btn btn-info btn-sm">
                                            <i class="fa fa-eye" aria-hidden="true"></i> View
                                        </button>
                                    </a>
                                    <a href="{{ url('/cardocuments/' . $item->id . '/edit') }}" title="Edit document">
                                    ...
                                    <form method="POST" action="{{ url('/cardocuments' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
                                        <button type="submit" class="btn btn-danger btn-sm" title="Delete document" onclick="return confirm(&quot;Confirm delete?&quot;)">
